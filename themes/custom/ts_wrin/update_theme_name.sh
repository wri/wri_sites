...
# If the theme still has the name ts_wrin sprinkled throughout, this does the
# job of changing those instances of 'ts_wrin' to the new theme name.
...
# It also renames the directory from ts_wrin to the new theme name if it is
...
#
  ...
  OLD_HUMAN_READABLE_NAME="Ts Wrin"

  echo Theme name is $THEME_NAME
  # assumes the currently running script is inside the ts_wrin directory that will be re-named
  ...
  # go through files and edit them replacing ts_wrin with the new theme name
  sed "s/ts_wrin_/${THEME_NAME}_/g" $THEME_DEST/ts_wrin.theme >$THEME_DEST/$THEME_NAME.theme
  rm $THEME_DEST/ts_wrin.theme

  sed "s/name: Ts Wrin/name: ${NEW_HUMAN_READABLE_NAME}/g" $THEME_DEST/ts_wrin.info.yml | \
  sed "s/ts_wrin/${THEME_NAME}/g" >$THEME_DEST/$THEME_NAME.info.yml
  rm $THEME_DEST/ts_wrin.info.yml

  sed "s/ts_wrin/${THEME_NAME}/g" $THEME_DEST/ts_wrin.yml >$THEME_DEST/$THEME_NAME.yml
  rm $THEME_DEST/ts_wrin.yml

  sed "s/ts_wrin/${THEME_NAME}/g" $THEME_DEST/ts_wrin.libraries.yml >$THEME_DEST/$THEME_NAME.libraries.yml
  rm $THEME_DEST/ts_wrin.libraries.yml

  mv $THEME_DEST/composer.json $THEME_DEST/composer.child
  sed "s/ts_wrin/${THEME_NAME}/g" $THEME_DEST/composer.child >$THEME_DEST/composer.json
  rm $THEME_DEST/composer.child

  mv $THEME_DEST/package.json $THEME_DEST/package.child
  sed "s/ts_wrin/${THEME_NAME}/g" $THEME_DEST/package.child >$THEME_DEST/package.json
  ...
  sed "s/ts_wrin/${THEME_NAME}/g" $THEME_DEST/README.md.child | \
  sed "s/ts_wrin/${THEME_NAME}/g" | \
  sed "s:/new-project-name/web/profiles/contrib/bene/themes/ts_wrin where new-project-name is your project.:${THEME_DEST}:g" | \
  sed "s:can be found in a project called \"new-project-name\" here\: /new-project-name/web/profiles/contrib/bene/themes/ts_wrin:can be found in a project called \"${PROJECT_NAME}\" here\: ${THEME_DEST}:g" | \
  ...
  sed "s/ts_wrin/${THEME_NAME}/g" update_theme_name.child | \
  sed "s/ts_wrin/${THEME_NAME}/g" | \
  sed "s/Ts Wrin/${NEW_HUMAN_READABLE_NAME}/g" | \
  sed "s/ts_wrin/${THEME_NAME}/g" >$THEME_DEST/update_theme_name.sh
